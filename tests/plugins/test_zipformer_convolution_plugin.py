...
ADDITIONAL_LAYOUT_CASES = (
    EngineCase("fp32-k15", trt.float32, cp.float32, 12, 15, 2e-5),
    EngineCase("fp16-k15", trt.float16, cp.float16, 24, 15, 2e-2),
    EngineCase(
        "bf16-k15",
        trt.bfloat16,
        cp.dtype("bfloat16"),
...
def convolution_engine(request, plugin_creator):
    ...
    *_, creator = plugin_creator
    ...
    runtime, engine = build_convolution_engine(
        creator,
...
def test_zipformer_convolution_plugin_supports_additional_layouts(
    ...
    """Cover alternate channel packing, clamped lengths, and kernel boundaries."""

    *_, creator = plugin_creator
    shape = (3, 17, engine_case.channels)
    _, engine = build_convolution_engine(
        creator,
        engine_case.trt_dtype,
        engine_case.channels,
        engine_case.kernel_size,
...
def test_zipformer_convolution_builds_with_constants(plugin_creator) -> None:
