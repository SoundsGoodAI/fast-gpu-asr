...
{
    return static_cast<int32_t>((static_cast<int64_t>(rows) + kWarpsPerBlock - 1) / kWarpsPerBlock);
}
...
{
    if (left < 1 || right < 1 || left > std::numeric_limits<int64_t>::max() / right)
    {
        return false;
    }
    output = left * right;
...
bool haveValidShapes(Dims const& projection, Dims const& position, Dims const& mask,
    Dims const& scores, DataType type) noexcept
{
    // projection: [N, T, H * (2 * query_dim + 4)]
    // position:   [1, 2 * T - 1, H, 4]
    // mask:       [N, T]
    // scores:     [N, H, T, T]
    if (projection.nbDims != 3 || position.nbDims != 4 || mask.nbDims != 2 || scores.nbDims != 4
        ...
        || !hasAddressableBytes(scores, dataTypeBytes(type)) || position.d[0] != 1
        || position.d[3] != kPositionHeadDim || mask.d[0] != projection.d[0]
        || mask.d[1] != projection.d[1] || scores.d[0] != projection.d[0]
        || scores.d[1] != position.d[2] || scores.d[2] != projection.d[1]
        || scores.d[3] != projection.d[1])
    {
        return false;
    }
    ...
    return dimensionsPerHead > kPositionHeadDim && (dimensionsPerHead - kPositionHeadDim) % 2 == 0;
}
...
template <typename T> __device__ float2 loadPair(T const* values);

template <> __device__ __forceinline__ float2 loadPair(half const* values)
{
    return __half22float2(*reinterpret_cast<half2 const*>(values));
}

template <> __device__ __forceinline__ float2 loadPair(__nv_bfloat16 const* values)
{
    return __bfloat1622float2(*reinterpret_cast<__nv_bfloat162 const*>(values));
}

template <> __device__ __forceinline__ float2 loadPair(float const* values)
{
...
__device__ __forceinline__ float blockMaximum(float value, float* warpValues)
{
    ...
    __syncthreads();
    value = threadIdx.x < (blockDim.x + kWarpSize - 1) / kWarpSize ? warpValues[lane] : -FLT_MAX;
    if (warp == 0)
    {
        value = warpMaximum(value);
    }
...
__device__ __forceinline__ float blockSum(float value, float* warpValues)
{
    ...
    __syncthreads();
    value = threadIdx.x < (blockDim.x + kWarpSize - 1) / kWarpSize ? warpValues[lane] : 0.0F;
    if (warp == 0)
    {
        value = warpSum(value);
    }
...
__global__ void softmaxWarpRows(T const* projection, T const* position, bool const* mask, T* scores,
    int32_t batchSize, int32_t numHeads, int32_t sequenceLength, int32_t queryDim)
{
    ...
    int32_t const projectionDim = numHeads * (2 * queryDim + kPositionHeadDim);
    int32_t const positionQueryOffset = 2 * numHeads * queryDim + head * kPositionHeadDim;
    ...
    if (query >= kPaddedQueryHalo && mask[batch * sequenceLength + query - kPaddedQueryHalo])
    {
        for (int32_t key = lane; key < sequenceLength; key += kWarpSize)
        {
            scores[static_cast<int64_t>(row) * sequenceLength + key] = fromFloat<T>(0.0F);
        }
        return;
    }
...
#pragma unroll
    for (int slot = 0; slot < ValuesPerThread; ++slot)
    {
        ...
        float value = -FLT_MAX;
        bool const valid =
            slot < slots && key < sequenceLength && !mask[batch * sequenceLength + key];
        if (valid)
        {
            ...
            int64_t const positionBase =
                (static_cast<int64_t>(relative) * numHeads + head) * kPositionHeadDim;
            ...
            value = toFloat(scores[static_cast<int64_t>(row) * sequenceLength + key])
                    + query01.x * position01.x + query01.y * position01.y + query23.x * position23.x
...
template <typename T>
__global__ void softmaxBlockGeneric(T const* projection, T const* position, bool const* mask,
    T* scores, int32_t numHeads, int32_t sequenceLength, int32_t queryDim)
{
    ...
    for (int32_t key = thread; key < sequenceLength; key += blockDim.x)
    {
        ...
        float const value = valid ? toFloat(scores[scoreBase + key]) + query01.x * position01.x
    ...
    for (int32_t key = thread; key < sequenceLength; key += blockDim.x)
    {
        float const value = mask[batch * sequenceLength + key]
                                ? 0.0F
                                : __expf(toFloat(scores[scoreBase + key]) - maximum);
        scores[scoreBase + key] = fromFloat<T>(value);
        sum += value;
    }
    ...
    for (int32_t key = thread; key < sequenceLength; key += blockDim.x)
    {
        scores[scoreBase + key] =
            fromFloat<T>(sum > 0.0F ? toFloat(scores[scoreBase + key]) / sum : 0.0F);
    }
}

template <typename T, int ValuesPerThread>
__global__ void softmaxBlockRegisters(T const* projection, T const* position, bool const* mask,
    T* scores, int32_t numHeads, int32_t sequenceLength, int32_t queryDim)
{
...
#pragma unroll
    for (int slot = 0; slot < ValuesPerThread; ++slot)
    {
        ...
        float value = -FLT_MAX;
        bool const valid = key < sequenceLength && !mask[batch * sequenceLength + key];
        if (valid)
        {
            int32_t const relative = sequenceLength - 1 - query + key;
            int64_t const positionBase =
            ...
            float2 const position01 = loadPair(position + positionBase);
            float2 const position23 = loadPair(position + positionBase + 2);
            value = toFloat(scores[scoreBase + key]) + query01.x * position01.x
                    + query01.y * position01.y + query23.x * position23.x
                    + query23.y * position23.y;
        }
...
#pragma unroll
    for (int slot = 0; slot < ValuesPerThread; ++slot)
    {
        ...
        {
            float const value =
                mask[batch * sequenceLength + key] ? 0.0F : __expf(values[slot] - maximum);
            sum += value;
...
#pragma unroll
    for (int slot = 0; slot < ValuesPerThread; ++slot)
    {
        ...
        if (key < sequenceLength)
        {
            scores[scoreBase + key] = fromFloat<T>(sum > 0.0F ? values[slot] / sum : 0.0F);
        }
    }
}

template <typename T> constexpr cudaDataType_t cudaDataType();

template <> constexpr cudaDataType_t cudaDataType<half>() { return CUDA_R_16F; }

template <> constexpr cudaDataType_t cudaDataType<__nv_bfloat16>() { return CUDA_R_16BF; }

template <> constexpr cudaDataType_t cudaDataType<float>() { return CUDA_R_32F; }

template <typename T>
int32_t enqueueRelativeAttention(cublasHandle_t cublas, int32_t tactic,
    PluginTensorDesc const* inputDesc, void const* const* inputs, void* const* outputs,
    cudaStream_t stream) noexcept
{
    ...
    int32_t const queryDim = (projectionDim / numHeads - kPositionHeadDim) / 2;

    float const alpha = 1.0F;
    float const beta = 0.0F;
    int64_t const projectionBatchStride = static_cast<int64_t>(sequenceLength) * projectionDim;
    ...
    for (int32_t head = 0; head < numHeads; ++head)
    {
        ...
        auto const* key = projection + numHeads * queryDim + head * queryDim;
        auto* headOutput = output + static_cast<int64_t>(head) * sequenceLength * sequenceLength;
        if (cublasGemmStridedBatchedEx(cublas, CUBLAS_OP_T, CUBLAS_OP_N, sequenceLength,
                sequenceLength, queryDim, &alpha, key, cudaDataType<T>(), projectionDim,
                projectionBatchStride, query, cudaDataType<T>(), projectionDim,
                projectionBatchStride, &beta, headOutput, cudaDataType<T>(), sequenceLength,
                outputBatchStride, batchSize, getCublasComputeType(tactic), CUBLAS_GEMM_DEFAULT)
    ...
    {
        softmaxWarpRows<T, 12><<<warpRowBlocks(rows), kThreadsPerBlock, 0, stream>>>(
            projection, position, mask, output, batchSize, numHeads, sequenceLength, queryDim);
    }
    else if (sequenceLength <= 512)
    {
        softmaxWarpRows<T, 16><<<warpRowBlocks(rows), kThreadsPerBlock, 0, stream>>>(
            projection, position, mask, output, batchSize, numHeads, sequenceLength, queryDim);
    }
    else if (sequenceLength <= 1024)
    {
        softmaxWarpRows<T, 32><<<warpRowBlocks(rows), kThreadsPerBlock, 0, stream>>>(
            projection, position, mask, output, batchSize, numHeads, sequenceLength, queryDim);
    }
    else if (sequenceLength <= 2048)
    {
        softmaxBlockRegisters<T, 4><<<rows, 512, 0, stream>>>(
            projection, position, mask, output, numHeads, sequenceLength, queryDim);
    }
    else
    {
...
{
  public:
    explicit RelativeAttentionPlugin(int32_t tactic = kStrictComputeTactic) noexcept
        : mTactic(tactic)
    {
        ...
        DynamicPluginTensorDesc const* outputs, int32_t nbOutputs) noexcept override
    {
        ...
        if (inputs == nullptr || outputs == nullptr || nbInputs != kInputCount
            ...
            || inputs[1].desc.dims.nbDims != 4 || inputs[2].desc.dims.nbDims != 2
            || outputs[0].desc.dims.nbDims != 4 || inputs[0].desc.type != inputs[1].desc.type
            || outputs[0].desc.type != inputs[0].desc.type
            || !isSupportedDataType(inputs[0].desc.type) || inputs[2].desc.type != DataType::kBOOL
            ...
            || !hasAddressableBytes(inputs[0].max, dataTypeBytes(inputs[0].desc.type))
            || !hasAddressableBytes(inputs[1].max, dataTypeBytes(inputs[1].desc.type))
        ...
        int32_t nbInputs) const noexcept override
    {
        if (outputTypes == nullptr || inputTypes == nullptr || nbInputs != kInputCount
            ...
            || inputTypes[1] != inputTypes[0] || inputTypes[2] != DataType::kBOOL)
        {
            return 1;
        }
        ...
        IExprBuilder& exprBuilder) noexcept override
    {
        ...
        if (inputs == nullptr || outputs == nullptr || nbInputs != kInputCount
            ...
            || inputs[1].nbDims != 4 || inputs[2].nbDims != 2 || inputs[0].d[0] == nullptr
    ...
    {
        if (inputs == nullptr || outputs == nullptr || nbInputs != kInputCount
            || nbOutputs != kOutputCount || !mInitialized || inputs[0].type != inputs[1].type
            || outputs[0].type != inputs[0].type || !isSupportedDataType(inputs[0].type)
            || inputs[2].type != DataType::kBOOL || inputs[0].format != TensorFormat::kLINEAR
            ...
            || !haveValidShapes(
                inputs[0].dims, inputs[1].dims, inputs[2].dims, outputs[0].dims, inputs[0].type))
        {
            return 1;
        }
        return 0;
        ...
        cudaStream_t stream) noexcept override
    {
        ...
        {
        case DataType::kHALF:
            return enqueueRelativeAttention<half>(
                mCublas, mTactic, inputDesc, inputs, outputs, stream);
        case DataType::kBF16:
            return enqueueRelativeAttention<__nv_bfloat16>(
                mCublas, mTactic, inputDesc, inputs, outputs, stream);
        case DataType::kFLOAT:
            return enqueueRelativeAttention<float>(
                mCublas, mTactic, inputDesc, inputs, outputs, stream);
        default: return 1;
        }
    }
...
extern "C" bool initFastGpuAsrZipformerRelativeAttentionPlugin() noexcept
{
    ...
    auto ensureRegistered =
        [](IPluginRegistry* registry, RelativeAttentionCreator& creator) noexcept
    {
        if (registry == nullptr)
        {
            return false;
