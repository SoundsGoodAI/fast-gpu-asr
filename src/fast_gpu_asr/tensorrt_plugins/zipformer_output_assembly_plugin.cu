    ...
    DataType type) noexcept
{
    ...
    if (inputs == nullptr || output.nbDims != 3
    ...
    {
        if (inputs[index].nbDims != 3
            || !hasAddressableBytes(inputs[index], elementBytes)
            || inputs[index].d[0] != inputs[0].d[0]
            || inputs[index].d[1] != inputs[0].d[1])
        {
            return false;
        }
    }

    if (output.d[0] != inputs[3].d[0]
        || output.d[1] != inputs[3].d[1]
    ...
    int64_t const encoder5Bytes =
        static_cast<int64_t>(inputs[4].d[2]) * elementBytes;
    int64_t const encoder6Bytes =
        static_cast<int64_t>(inputs[5].d[2]) * elementBytes;
    return inputs[3].d[2] >= inputs[4].d[2]
        && inputs[4].d[2] >= inputs[5].d[2]
        && encoder4Bytes % kVectorBytes == 0
        && encoder5Bytes % kVectorBytes == 0
...
__global__ void assembleOutput(uint4 const* __restrict__ encoder4,
    uint4 const* __restrict__ encoder5,
    uint4 const* __restrict__ encoder6, uint4* __restrict__ output,
    int64_t totalVectors, int32_t outputVectors, int32_t encoder5Vectors,
    int32_t encoder6Vectors)
{
...
public:
        ...
        IExprBuilder& exprBuilder) noexcept override
    {
        ...
        for (int32_t index = 0; index < kInputCount; ++index)
        {
            if (inputs[index].nbDims != 3 || inputs[index].d[0] == nullptr
                || inputs[index].d[1] == nullptr
                || inputs[index].d[2] == nullptr)
            {
                return 1;
            }
        }
        ...
        cudaStream_t stream) noexcept override
    {
        ...
        if (!isAligned(inputs[3]) || !isAligned(inputs[4])
            || !isAligned(inputs[5]) || !isAligned(outputs[0]))
        {
            return 1;
        }

        int32_t const elementBytes = dataTypeBytes(inputDesc[0].type);
        int64_t const outputBytes =
            static_cast<int64_t>(inputDesc[3].dims.d[2]) * elementBytes;
        int64_t const encoder5Bytes =
            static_cast<int64_t>(inputDesc[4].dims.d[2]) * elementBytes;
        int64_t const encoder6Bytes =
            static_cast<int64_t>(inputDesc[5].dims.d[2]) * elementBytes;
        ...
        if (outputVectorCount < 1
            || outputVectorCount > std::numeric_limits<int32_t>::max()
            || encoder5VectorCount < 1
            || encoder5VectorCount > std::numeric_limits<int32_t>::max()
            || encoder6VectorCount < 1
            || encoder6VectorCount > std::numeric_limits<int32_t>::max())
        {
            return 1;
        }
        ...
        int32_t const outputVectors =
            static_cast<int32_t>(outputVectorCount);
        int32_t const encoder5Vectors =
            static_cast<int32_t>(encoder5VectorCount);
        int32_t const encoder6Vectors =
            static_cast<int32_t>(encoder6VectorCount);

        // Batch and time do not affect the band mapping, so launch one
        // grid-stride copy over all 16-byte vectors in all NTC rows.
        int64_t const frames = static_cast<int64_t>(inputDesc[3].dims.d[0])
            * inputDesc[3].dims.d[1];
        ...
        int64_t const requiredBlocks =
            (totalVectors - 1) / kThreadsPerBlock + 1;
        ...
        assembleOutput<<<blocks, kThreadsPerBlock, 0, stream>>>(
            ...
            static_cast<uint4 const*>(inputs[4]),
            static_cast<uint4 const*>(inputs[5]),
            static_cast<uint4*>(outputs[0]), totalVectors, outputVectors,
            encoder5Vectors, encoder6Vectors);
...
{
public:
    OutputAssemblyCreator() noexcept
    {
        mFields = {0, nullptr};
    }
...
extern "C" bool initFastGpuAsrZipformerOutputAssemblyPlugin() noexcept
{
    ...
    auto ensureRegistered = [](IPluginRegistry* registry,
                                OutputAssemblyCreator& creator) noexcept {
        if (registry == nullptr)
        {
            return false;
        }
