...
bool haveValidShapes(Dims const& attention, Dims const& value, Dims const& output,
    int32_t valueHeads, int32_t elementBytes) noexcept
{
    if (valueHeads < 1 || attention.nbDims != 4 || value.nbDims != 3 || output.nbDims != 3
        || !hasAddressableBytes(attention, elementBytes)
        || !hasAddressableBytes(value, elementBytes) || !hasAddressableBytes(output, elementBytes)
        || attention.d[1] < valueHeads || (valueHeads > 1 && attention.d[1] != valueHeads)
        || attention.d[0] != value.d[0] || attention.d[2] != attention.d[3]
        || attention.d[2] != value.d[1] || value.d[2] % valueHeads != 0
        || attention.d[0] > std::numeric_limits<int32_t>::max() / valueHeads
        || output.d[0] != value.d[0] || output.d[1] != value.d[1] || output.d[2] != value.d[2])
    {
        return false;
    }
    return true;
...
template <typename T>
__global__ void attentionValueHead12(T const* attentionWeights, T const* value, T* output,
    int32_t batchSize, int32_t attentionHeads, int32_t valueHeads, int32_t sequenceLength,
    int32_t channels)
{
    ...
    int64_t const warp = (static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x) / warpSize;
    int64_t const warpStride = static_cast<int64_t>(blockDim.x) * gridDim.x / warpSize;
    int64_t const rowCount = static_cast<int64_t>(batchSize) * valueHeads * sequenceLength;
    ...
    for (int64_t row = warp; row < rowCount; row += warpStride)
    {
        ...
        int64_t const attentionOffset =
            (static_cast<int64_t>(batch) * attentionHeads + head) * sequenceLength * sequenceLength
            + static_cast<int64_t>(query) * sequenceLength;
        int64_t const valueOffset =
            static_cast<int64_t>(batch) * sequenceLength * channels + head * kNarrowHeadDim;

        float sums[kNarrowHeadDim]{};
        for (int64_t key = lane; key < sequenceLength; key += warpSize)
        {
            float const weight = toFloat(attentionWeights[attentionOffset + key]);
            int64_t const keyOffset = valueOffset + key * channels;
#pragma unroll
            for (int32_t channel = 0; channel < kNarrowHeadDim; ++channel)
            {
                sums[channel] += weight * toFloat(value[keyOffset + channel]);
            }
        }
...
#pragma unroll
            for (int32_t channel = 0; channel < kNarrowHeadDim; ++channel)
            {
                sums[channel] += __shfl_down_sync(0xFFFFFFFFU, sums[channel], offset);
            }
        }
        ...
        if (lane == 0)
        {
            int64_t const outputOffset =
                (static_cast<int64_t>(batch) * sequenceLength + query) * channels
                + head * kNarrowHeadDim;
#pragma unroll
            for (int32_t channel = 0; channel < kNarrowHeadDim; ++channel)
            {
                output[outputOffset + channel] = fromFloat<T>(sums[channel]);
            }
        }
    }
}

__global__ void initializeAttentionValuePointers(void const* attentionWeights, void const* value,
    void* output, void const** attentionPointers, void const** valuePointers, void** outputPointers,
    int32_t batchCount, int32_t attentionHeads, int32_t valueHeads, int32_t sequenceLength,
    int32_t channels, int32_t headDim, int32_t elementBytes)
{
    ...
    int64_t const thread = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
...
{
  public:
    explicit AttentionValuePlugin(int32_t valueHeads = 1, int32_t tactic = kStrictComputeTactic,
          ...
          mSupportsNarrowHeadTactic(supportsNarrowHeadTactic)
    {
        ...
        auto const result = std::to_chars(mTimingCacheId.data() + prefix.size(),
        ...
        mSerializedField = {kNumHeadsField, &mValueHeads, PluginFieldType::kINT32, 1};
        mFields = {1, &mSerializedField};
        mInitialized =
            valueHeads > 0 && validTimingCacheId && cublasCreate(&mHandle) == CUBLAS_STATUS_SUCCESS;
    }
    ...
    IPluginV3* clone() noexcept override
    {
        auto* plugin = new (std::nothrow)
            AttentionValuePlugin(mValueHeads, mTactic, mSupportsNarrowHeadTactic);
        ...
        DynamicPluginTensorDesc const* outputs, int32_t nbOutputs) noexcept override
    {
        if (inputs == nullptr || outputs == nullptr || nbInputs != kInputCount
            || nbOutputs != kOutputCount || inputs[0].desc.dims.nbDims != 4
            || inputs[1].desc.dims.nbDims != 3 || outputs[0].desc.dims.nbDims != 3
            || mValueHeads < 1 || inputs[0].desc.type != inputs[1].desc.type
            ...
            || !haveValidShapes(inputs[0].min, inputs[1].min, outputs[0].min, mValueHeads,
                dataTypeBytes(inputs[1].desc.type))
            || !haveValidShapes(inputs[0].opt, inputs[1].opt, outputs[0].opt, mValueHeads,
                dataTypeBytes(inputs[1].desc.type))
            || !haveValidShapes(inputs[0].max, inputs[1].max, outputs[0].max, mValueHeads,
                dataTypeBytes(inputs[1].desc.type))
            || !mInitialized)
        {
            return 1;
        ...
        mSupportsNarrowHeadTactic = mValueHeads > 1 && channels > 0 && channels % mValueHeads == 0
    ...
    {
        if (inputs == nullptr || outputs == nullptr || nbInputs != kInputCount
            || nbOutputs != kOutputCount || mValueHeads < 1 || inputs[1].max.d[0] < 1
            || inputs[1].max.d[0] > std::numeric_limits<int32_t>::max() / mValueHeads)
        {
            return 0;
        }
        ...
        if (maxBatchSize > std::numeric_limits<size_t>::max() / valueHeads / bytesPerPointerSet)
        {
            return 0;
        }
    ...
    int32_t getValidTactics(int32_t* tactics, int32_t nbTactics) noexcept override
    {
        ...
        if (tactics == nullptr
            || nbTactics != cublasTactics + static_cast<int32_t>(mSupportsNarrowHeadTactic)
            || writeCublasComputeTactics(tactics, cublasTactics, mInputType, ampereCompute) != 0)
        {
            return 1;
        }
    ...
    char const* getTimingCacheID() noexcept override { return mTimingCacheId.data(); }
        ...
        PluginTensorDesc const* outputs, int32_t nbOutputs) noexcept override
    {
        ...
        if (inputs == nullptr || outputs == nullptr || nbInputs != kInputCount
            || nbOutputs != kOutputCount || mValueHeads < 1 || !mInitialized
            || inputs[0].type != inputs[1].type || outputs[0].type != inputs[1].type
            || !isSupportedDataType(inputs[1].type) || inputs[0].format != TensorFormat::kLINEAR
            || inputs[1].format != TensorFormat::kLINEAR
            || outputs[0].format != TensorFormat::kLINEAR
            || !haveValidShapes(inputs[0].dims, inputs[1].dims, outputs[0].dims, mValueHeads,
                dataTypeBytes(inputs[1].type))
            || (mTactic != kNarrowHeadTactic && !isCublasComputeTactic(mTactic, inputs[1].type))
            || (mTactic == kNarrowHeadTactic
                && (mValueHeads == 1 || inputs[1].dims.d[2] / mValueHeads != kNarrowHeadDim)))
        {
            return 1;
        }
        return 0;
        ...
        cudaStream_t stream) noexcept override
    {
        if (inputDesc == nullptr || outputDesc == nullptr || inputs == nullptr || outputs == nullptr
            ...
            || !mInitialized || inputDesc[0].type != inputDesc[1].type
            ...
            || !haveValidShapes(inputDesc[0].dims, inputDesc[1].dims, outputDesc[0].dims,
                mValueHeads, dataTypeBytes(inputDesc[1].type))
            || (mTactic != kNarrowHeadTactic && !isCublasComputeTactic(mTactic, inputDesc[1].type))
            || (mTactic == kNarrowHeadTactic
                && (mValueHeads == 1 || inputDesc[1].dims.d[2] / mValueHeads != kNarrowHeadDim)))
        {
            return 1;
        }
        ...
        int64_t const batchCount64 = static_cast<int64_t>(batchSize) * mValueHeads;
        if (batchCount64 < 1 || batchCount64 > std::numeric_limits<int32_t>::max())
        {
            return 1;
        }
        ...
        if (mTactic == kNarrowHeadTactic)
        {
            ...
            int32_t const blocks = static_cast<int32_t>(std::min<int64_t>(
                kMaxBlocks, rows / kWarpsPerBlock + (rows % kWarpsPerBlock != 0)));
            if (inputDesc[1].type == DataType::kFLOAT)
            {
                attentionValueHead12<<<blocks, kThreads, 0, stream>>>(
                    ...
                    static_cast<float*>(outputs[0]), batchSize, attentionHeads, mValueHeads,
            ...
            else if (inputDesc[1].type == DataType::kHALF)
            {
                attentionValueHead12<<<blocks, kThreads, 0, stream>>>(
                    ...
                    static_cast<half*>(outputs[0]), batchSize, attentionHeads, mValueHeads,
                    sequenceLength, channels);
            }
            else if (inputDesc[1].type == DataType::kBF16)
            {
                attentionValueHead12<<<blocks, kThreads, 0, stream>>>(
                    ...
                    static_cast<__nv_bfloat16*>(outputs[0]), batchSize, attentionHeads, mValueHeads,
                    sequenceLength, channels);
            }
            else
            {
        ...
        {
        case DataType::kFLOAT: dataType = CUDA_R_32F; break;
        case DataType::kHALF: dataType = CUDA_R_16F; break;
        case DataType::kBF16: dataType = CUDA_R_16BF; break;
        default: return 1;
        }
        ...
        int32_t const pointerBlocks = static_cast<int32_t>(
            std::min<int64_t>(kMaxBlocks, (batchCount64 + kPointerThreads - 1) / kPointerThreads));
        initializeAttentionValuePointers<<<pointerBlocks, kPointerThreads, 0, stream>>>(inputs[0],
            inputs[1], outputs[0], attentionPointers, valuePointers, outputPointers, batchCount,
            attentionHeads, mValueHeads, sequenceLength, channels, headDim, elementBytes);
        ...
        cublasStatus_t const status = cublasGemmBatchedEx(mHandle, CUBLAS_OP_N, CUBLAS_OP_N,
            headDim, sequenceLength, sequenceLength, &alpha, valuePointers, dataType, channels,
            attentionPointers, dataType, sequenceLength, &beta, outputPointers, dataType, channels,
            batchCount, getCublasComputeType(mTactic), CUBLAS_GEMM_DEFAULT);
        return status == CUBLAS_STATUS_SUCCESS && cudaGetLastError() == cudaSuccess ? 0 : 1;
    }
  ...
  public:
        ...
        TensorRTPhase phase) noexcept override
    {
        ...
        for (int32_t index = 0; index < fields->nbFields; ++index)
        {
            ...
            if (field.name != nullptr && std::string_view(field.name) == kNumHeadsField)
            {
                if (foundValueHeads || field.type != PluginFieldType::kINT32 || field.length != 1
                    || field.data == nullptr)
                {
                    return nullptr;
                }
...
extern "C" bool initFastGpuAsrZipformerAttentionValuePlugin() noexcept
{
    ...
    auto ensureRegistered =
        [](IPluginRegistry* registry, AttentionValuePluginCreator& creator) noexcept
    {
        if (registry == nullptr)
        {
            return false;
