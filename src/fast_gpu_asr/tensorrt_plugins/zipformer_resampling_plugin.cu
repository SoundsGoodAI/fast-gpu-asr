...
bool haveValidDownsampleShapes(Dims const& input, Dims const& weights, Dims const& output,
    int32_t factor, int32_t elementBytes) noexcept
{
    if (factor < 1 || input.nbDims != 3 || weights.nbDims != 2 || output.nbDims != 3
        || !hasAddressableBytes(input, elementBytes) || !hasAddressableBytes(weights, elementBytes)
...
{
    if (factor < 1 || early.nbDims != 3 || later.nbDims != 3 || scale.nbDims != 1
        || output.nbDims != 3 || !hasAddressableBytes(early, elementBytes)
        || !hasAddressableBytes(later, elementBytes) || !hasAddressableBytes(scale, elementBytes)
        || !hasAddressableBytes(output, elementBytes))
    {
        return false;
    }
    ...
    DataType type) noexcept
{
    ...
    return operation == Operation::kDownsample
               ? haveValidDownsampleShapes(inputs[0], inputs[1], output, factor, elementBytes)
               : haveValidUpsampleShapes(
                     inputs[0], inputs[1], inputs[2], output, factor, elementBytes);
}

template <typename T> __device__ __forceinline__ float toFloat(T value);

template <> __device__ __forceinline__ float toFloat(float value) { return value; }

template <> __device__ __forceinline__ float toFloat(half value) { return __half2float(value); }

template <> __device__ __forceinline__ float toFloat(__nv_bfloat16 value)
{
    return __bfloat162float(value);
}

template <typename T> __device__ __forceinline__ T fromFloat(float value);
...
template <typename T>
__global__ void downsample(T const* input, T const* weights, T* output, int32_t inputLength,
    int32_t outputLength, int32_t channels, int32_t factor)
{
    ...
    int64_t const channel = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (channel >= channels)
    {
        return;
    }
    ...
    int64_t const firstInputFrame = static_cast<int64_t>(outputFrame) * factor;
    float value = 0.0F;
    for (int64_t offset = 0; offset < factor; ++offset)
    {
        int64_t const candidateFrame = firstInputFrame + offset;
        int32_t const inputFrame =
            candidateFrame < inputLength ? static_cast<int32_t>(candidateFrame) : inputLength - 1;
        int64_t const inputIndex =
            (static_cast<int64_t>(batch) * inputLength + inputFrame) * channels + channel;
...
__global__ void downsampleHalf2(half2 const* input, half const* weights, half2* output,
    int32_t inputLength, int32_t outputLength, int32_t channelPairs, int32_t factor)
{
    ...
    int64_t const channelPair = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    ...
    for (int64_t offset = 0; offset < factor; ++offset)
    {
        ...
        int32_t const inputFrame =
            candidateFrame < inputLength ? static_cast<int32_t>(candidateFrame) : inputLength - 1;
        int64_t const inputIndex =
            (static_cast<int64_t>(batch) * inputLength + inputFrame) * channelPairs + channelPair;
...
__global__ void downsampleBfloat162(__nv_bfloat162 const* input, __nv_bfloat16 const* weights,
    __nv_bfloat162* output, int32_t inputLength, int32_t outputLength, int32_t channelPairs,
    int32_t factor)
{
    int64_t const channelPair = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (channelPair >= channelPairs)
...
template <typename T>
__global__ void upsampleBypass(T const* early, T const* later, T const* scale, T* output,
    int32_t outputLength, int32_t laterLength, int32_t channels, int32_t factor)
{
    ...
    int64_t const laterIndex =
        (static_cast<int64_t>(batch) * laterLength + laterFrame) * channels + channel;
    ...
    output[outputIndex] = fromFloat<T>(earlyValue + (laterValue - earlyValue) * channelScale);
}

__global__ void upsampleBypassHalf2(half2 const* early, half2 const* later, half2 const* scale,
    half2* output, int32_t outputLength, int32_t laterLength, int32_t channelPairs, int32_t factor)
{
    int64_t const channelPair = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (channelPair >= channelPairs)
    {
    ...
    int64_t const outputIndex =
        (static_cast<int64_t>(batch) * outputLength + outputFrame) * channelPairs + channelPair;
    int64_t const laterIndex =
        (static_cast<int64_t>(batch) * laterLength + laterFrame) * channelPairs + channelPair;
    ...
    output[outputIndex] =
        __floats2half2_rn(earlyValue.x + (laterValue.x - earlyValue.x) * channelScale.x,
            earlyValue.y + (laterValue.y - earlyValue.y) * channelScale.y);
}

__global__ void upsampleBypassBfloat162(__nv_bfloat162 const* early, __nv_bfloat162 const* later,
    __nv_bfloat162 const* scale, __nv_bfloat162* output, int32_t outputLength, int32_t laterLength,
    int32_t channelPairs, int32_t factor)
{
    ...
    output[outputIndex] =
        __floats2bfloat162_rn(earlyValue.x + (laterValue.x - earlyValue.x) * channelScale.x,
            earlyValue.y + (laterValue.y - earlyValue.y) * channelScale.y);
}
                               ...
                               public IPluginV3OneRuntime
{
  public:
    ...
    {
        mSerializedField = {kFactorField, &mFactor, PluginFieldType::kINT32, 1};
    ...
    {
        return mOperation == Operation::kDownsample ? kDownsampleName : kUpsampleName;
    }
        ...
        DynamicPluginTensorDesc const* outputs, int32_t nbOutputs) noexcept override
    {
        ...
        if (inputs == nullptr || outputs == nullptr || nbInputs != expectedInputs
            || nbOutputs != kOutputCount || mFactor < 1 || inputs[0].desc.dims.nbDims != 3
            || outputs[0].desc.dims.nbDims != 3
            || (mOperation == Operation::kDownsample && inputs[1].desc.dims.nbDims != 2)
            || (mOperation == Operation::kUpsample
                && (inputs[1].desc.dims.nbDims != 3 || inputs[2].desc.dims.nbDims != 1))
        ...
        return haveValidShapes(mOperation, minInputs, outputs[0].min, mFactor, inputs[0].desc.type)
                       && haveValidShapes(
                           mOperation, optInputs, outputs[0].opt, mFactor, inputs[0].desc.type)
                       && haveValidShapes(
                           mOperation, maxInputs, outputs[0].max, mFactor, inputs[0].desc.type)
                   ? 0
                   : 1;
    }
    ...
    {
        int32_t const expectedInputs = expectedInputCount(mOperation);
        if (outputTypes == nullptr || inputTypes == nullptr || nbInputs != expectedInputs
        ...
        IExprBuilder& exprBuilder) noexcept override
    {
        ...
        int32_t const expectedInputs = expectedInputCount(mOperation);
        if (inputs == nullptr || outputs == nullptr || nbInputs != expectedInputs
            || nbOutputs != kOutputCount || nbShapeInputs != 0 || mFactor < 1
            || inputs[0].nbDims != 3 || inputs[0].d[0] == nullptr || inputs[0].d[1] == nullptr
            || inputs[0].d[2] == nullptr
            || (mOperation == Operation::kDownsample && inputs[1].nbDims != 2)
            || (mOperation == Operation::kUpsample
        ...
        if (mOperation == Operation::kDownsample)
        {
            ...
            auto* const adjustedLength =
                exprBuilder.operation(DimensionOperation::kSUB, *inputs[0].d[1], *one);
            if (adjustedLength == nullptr)
            {
                return 1;
            }
            auto* const quotient =
                exprBuilder.operation(DimensionOperation::kFLOOR_DIV, *adjustedLength, *divisor);
            if (quotient == nullptr)
            {
                return 1;
            }
            auto* const outputLength =
                exprBuilder.operation(DimensionOperation::kSUM, *quotient, *one);
    ...
    {
        int32_t const expectedInputs = expectedInputCount(mOperation);
        if (inOut == nullptr || nbInputs != expectedInputs || nbOutputs != kOutputCount || pos < 0
        ...
        PluginTensorDesc const* outputs, int32_t nbOutputs) noexcept override
    {
        ...
        if (inputs == nullptr || outputs == nullptr || nbInputs != expectedInputs
            ...
            || outputs[0].type != inputs[0].type || inputs[0].format != TensorFormat::kLINEAR
        ...
        return haveValidShapes(mOperation, inputShapes, outputs[0].dims, mFactor, inputs[0].type)
                   ? 0
                   : 1;
    }
        ...
        cudaStream_t stream) noexcept override
    {
        ...
        if (onShapeChange(inputDesc, expectedInputs, outputDesc, kOutputCount) != 0)
        {
            return 1;
        }
        ...
        int32_t const inputLength = static_cast<int32_t>(inputDesc[0].dims.d[1]);
        int32_t const channels = static_cast<int32_t>(inputDesc[0].dims.d[2]);
        int32_t const outputLength = static_cast<int32_t>(outputDesc[0].dims.d[1]);
        ...
        if ((inputDesc[0].type == DataType::kHALF || inputDesc[0].type == DataType::kBF16)
            && channels % 2 == 0)
        {
            int32_t const channelPairs = channels / 2;
            uint32_t const channelBlocks = static_cast<uint32_t>(
                (static_cast<int64_t>(channelPairs) + kThreadsPerBlock - 1) / kThreadsPerBlock);
            ...
            if (inputDesc[0].type == DataType::kHALF)
            {
                if (mOperation == Operation::kDownsample)
                {
                    downsampleHalf2<<<grid, block, 0, stream>>>(
                        static_cast<half2 const*>(inputs[0]), static_cast<half const*>(inputs[1]),
                        static_cast<half2*>(outputs[0]), inputLength, outputLength, channelPairs,
                        mFactor);
                }
                else
                {
                    upsampleBypassHalf2<<<grid, block, 0, stream>>>(
                        static_cast<half2 const*>(inputs[0]), static_cast<half2 const*>(inputs[1]),
                        static_cast<half2 const*>(inputs[2]), static_cast<half2*>(outputs[0]),
                        outputLength, static_cast<int32_t>(inputDesc[1].dims.d[1]), channelPairs,
                        mFactor);
                }
            }
            else if (mOperation == Operation::kDownsample)
            {
                downsampleBfloat162<<<grid, block, 0, stream>>>(
                    ...
                    static_cast<__nv_bfloat162*>(outputs[0]), inputLength, outputLength,
            ...
            else
            {
                upsampleBypassBfloat162<<<grid, block, 0, stream>>>(
                    ...
                    static_cast<int32_t>(inputDesc[1].dims.d[1]), channelPairs, mFactor);
            }
        }
        else
        {
            uint32_t const channelBlocks = static_cast<uint32_t>(
                (static_cast<int64_t>(channels) + kThreadsPerBlock - 1) / kThreadsPerBlock);
            dim3 const grid(channelBlocks, outputLength, batch);
            ...
            case DataType::kHALF:
                if (mOperation == Operation::kDownsample)
                {
                    downsample<<<grid, block, 0, stream>>>(static_cast<half const*>(inputs[0]),
                        static_cast<half const*>(inputs[1]), static_cast<half*>(outputs[0]),
                        inputLength, outputLength, channels, mFactor);
                }
                else
                {
                    upsampleBypass<<<grid, block, 0, stream>>>(static_cast<half const*>(inputs[0]),
                        static_cast<half const*>(inputs[1]), static_cast<half const*>(inputs[2]),
                        static_cast<half*>(outputs[0]), outputLength,
                        static_cast<int32_t>(inputDesc[1].dims.d[1]), channels, mFactor);
                }
                break;
            case DataType::kBF16:
                if (mOperation == Operation::kDownsample)
                {
                    downsample<<<grid, block, 0, stream>>>(
                        ...
                        static_cast<__nv_bfloat16 const*>(inputs[1]),
                        static_cast<__nv_bfloat16*>(outputs[0]), inputLength, outputLength,
                        channels, mFactor);
                }
                else
                {
            ...
            case DataType::kFLOAT:
                if (mOperation == Operation::kDownsample)
                {
                    downsample<<<grid, block, 0, stream>>>(static_cast<float const*>(inputs[0]),
                        static_cast<float const*>(inputs[1]), static_cast<float*>(outputs[0]),
                        inputLength, outputLength, channels, mFactor);
                }
                else
                {
                    upsampleBypass<<<grid, block, 0, stream>>>(static_cast<float const*>(inputs[0]),
                        static_cast<float const*>(inputs[1]), static_cast<float const*>(inputs[2]),
...
{
  public:
    explicit ResamplingCreator(Operation operation) noexcept : mOperation(operation)
    {
        ...
        TensorRTPhase phase) noexcept override
    {
        ...
        for (int32_t index = 0; index < fields->nbFields; ++index)
        {
            ...
            if (field.name != nullptr && std::string_view(field.name) == kFactorField)
            {
                if (foundFactor || field.type != PluginFieldType::kINT32 || field.length != 1
                    || field.data == nullptr)
                {
                    return nullptr;
                }
...
extern "C" bool initFastGpuAsrZipformerResamplingPlugins() noexcept
{
    ...
    auto ensureRegistered = [](IPluginRegistry* registry, ResamplingCreator& creator) noexcept
    {
        if (registry == nullptr)
        {
            return false;
        }
        if (registry->getCreator(creator.getPluginName(), kPluginVersion, kPluginNamespace)
            != nullptr)
        {
            return true;
        }
        return registry->registerCreator(creator, kPluginNamespace)
               || registry->getCreator(creator.getPluginName(), kPluginVersion, kPluginNamespace)
                      != nullptr;
    };

    auto* runtimeRegistry = getPluginRegistry();
    bool const runtimeRegistered = ensureRegistered(runtimeRegistry, runtimeDownsample)
    ...
    bool const builderRegistered = ensureRegistered(builderRegistry, builderDownsample)
