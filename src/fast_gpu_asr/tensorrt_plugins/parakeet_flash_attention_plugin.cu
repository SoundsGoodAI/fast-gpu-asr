...
constexpr char const* kTimingCacheId =
    "layout=nt3c-ntc;relative=2t-1;queries=fused;softmax=warp4";
...
bool hasAddressableBytes(Dims const& dims, int32_t elementBytes) noexcept
{
    ...
    for (int32_t index = 0; index < dims.nbDims; ++index)
    {
        if (dims.d[index] < 1
            || !checkedMultiply(
                elements, static_cast<size_t>(dims.d[index]), elements))
        {
            return false;
        }
    }
...
bool computeWorkspaceLayout(Dims const& qkv, Dims const& position,
    Dims const& bias, DataType type, WorkspaceLayout& layout) noexcept
{
    int32_t const elementBytes = dataTypeBytes(type);
    if (qkv.nbDims != 3 || position.nbDims != 3 || bias.nbDims != 2
        || elementBytes == 0)
    {
        return false;
    }
    ...
    size_t offset{};
    if (!checkedMultiply(static_cast<size_t>(qkv.d[0]),
            static_cast<size_t>(bias.d[0]), batchHeads)
        || !checkedMultiply(static_cast<size_t>(qkv.d[0]),
            static_cast<size_t>(qkv.d[1]), queryElements)
        || !checkedMultiply(queryElements, static_cast<size_t>(position.d[2]),
            queryElements)
        || !checkedMultiply(queryElements, static_cast<size_t>(elementBytes),
            queryBytes)
        || !checkedMultiply(batchHeads, static_cast<size_t>(qkv.d[1]),
            positionElements)
        || !checkedMultiply(positionElements,
            static_cast<size_t>(position.d[1]), positionElements)
        || !checkedMultiply(positionElements,
            static_cast<size_t>(elementBytes), positionBytes)
        || !checkedMultiply(batchHeads, static_cast<size_t>(qkv.d[1]),
            attentionElements)
        || !checkedMultiply(attentionElements,
            static_cast<size_t>(qkv.d[1]), attentionElements)
        || !checkedMultiply(attentionElements,
            static_cast<size_t>(elementBytes), attentionBytes)
        || !checkedMultiply(batchHeads, kPointerArrayCount * sizeof(void*),
            pointerBytes))
    {
        return false;
    }
    ...
    if (!checkedAdd(offset, queryBytes, offset)
        || !checkedAlignUp(offset, 256, offset))
    {
        return false;
    }
    ...
    if (!checkedAdd(offset, positionBytes, offset)
        || !checkedAlignUp(offset, 256, offset))
    {
        return false;
    }
    layout.attentionWeightsOffset = offset;
    if (!checkedAdd(offset, attentionBytes, offset)
        || !checkedAlignUp(offset, 256, offset))
    {
        return false;
    }
    layout.pointerArraysOffset = offset;
    if (!checkedAdd(offset, pointerBytes, offset)
        || !checkedAlignUp(offset, 256, offset))
    {
        return false;
    }
    ...
    return layout.totalBytes <= static_cast<size_t>(
...
{
    if (inputs == nullptr || inputs[0].nbDims != 3 || inputs[1].nbDims != 3
        || inputs[2].nbDims != 2 || inputs[3].nbDims != 2
        || inputs[4].nbDims != 1 || output.nbDims != 3
    ...
    if (inputs[0].d[2] != 3 * channels || heads * headDim != channels
        || batch > std::numeric_limits<int32_t>::max() / heads
        || inputs[1].d[0] != 1 || inputs[1].d[1] != relativeLength
        || !sameShape(inputs[2], inputs[3]) || inputs[4].d[0] != batch
        || output.d[0] != batch || output.d[1] != length
        || output.d[2] != channels)
    {
        return false;
    }
...
template <>
__device__ __forceinline__ half addQueryBias<half>(half query, half bias)
{
    ...
    __nv_bfloat16 query, __nv_bfloat16 bias)
{
    return __float2bfloat16(
        __bfloat162float(query) + __bfloat162float(bias));
}

template <typename T>
__global__ void prepareQueriesAndPositionPointers(T const* qkv,
    T const* position, T const* contentBias, T const* positionBias,
    T* contentQuery, T* positionQuery, T* positionScores,
    void const** positionPointers, void const** queryPointers,
    void** outputPointers, int32_t batchSize, int32_t numHeads,
    int32_t sequenceLength, int32_t relativeLength, int32_t channels,
    int32_t headDim)
{
    int64_t const pointerCount =
        static_cast<int64_t>(batchSize) * numHeads;
    int64_t const queryCount =
        static_cast<int64_t>(batchSize) * sequenceLength * channels;
    ...
    for (int64_t index = first; index < pointerCount; index += stride)
    {
        ...
        positionPointers[index] = position
            + static_cast<int64_t>(head) * headDim;
        queryPointers[index] = positionQuery
            + index * sequenceLength * headDim;
        outputPointers[index] = positionScores
            + index * sequenceLength * relativeLength;
    }
    for (int64_t index = first; index < queryCount; index += stride)
    {
        int64_t const batchStride =
            static_cast<int64_t>(sequenceLength) * channels;
        int32_t const batch = static_cast<int32_t>(index / batchStride);
        int64_t const indexInBatch = index - batch * batchStride;
        int32_t const frame =
            static_cast<int32_t>(indexInBatch / channels);
        int32_t const channel =
            static_cast<int32_t>(indexInBatch - frame * channels);
        int32_t const head = channel / headDim;
        int32_t const headChannel = channel - head * headDim;
        int64_t const queryOffset =
            (static_cast<int64_t>(batch) * sequenceLength + frame)
                * 3 * channels
            + channel;
        int64_t const outputOffset =
            ((static_cast<int64_t>(batch) * numHeads + head)
                    * sequenceLength
                + frame)
                * headDim
            + headChannel;
        T const query = qkv[queryOffset];
        contentQuery[outputOffset] =
            addQueryBias(query, contentBias[channel]);
        positionQuery[outputOffset] =
            addQueryBias(query, positionBias[channel]);
    }
}

__global__ void initializeContentPointers(void const* qkv,
    void const* contentQuery, void* contentScores,
    void const** keyPointers, void const** queryPointers,
    void** outputPointers, int32_t batchSize, int32_t numHeads,
    int32_t sequenceLength, int32_t channels, int32_t headDim,
    int32_t elementBytes)
{
    ...
    for (int64_t index = first; index < count; index += stride)
    {
        ...
        auto const* queryBytes =
            static_cast<unsigned char const*>(contentQuery);
        auto* outputBytes = static_cast<unsigned char*>(contentScores);
        int64_t const keyOffset =
            ((static_cast<int64_t>(batch) * sequenceLength * 3 + 1)
                    * channels
                + static_cast<int64_t>(head) * headDim)
            * elementBytes;
        keyPointers[index] = qkvBytes + keyOffset;
        queryPointers[index] = queryBytes
            + index * sequenceLength * headDim * elementBytes;
        outputPointers[index] = outputBytes
            + index * sequenceLength * sequenceLength * elementBytes;
    }
}

__global__ void initializeValuePointers(void const* qkv,
    void const* attentionWeights, void* output,
    void const** valuePointers, void const** attentionPointers,
    void** outputPointers, int32_t batchSize, int32_t numHeads,
    int32_t sequenceLength, int32_t channels, int32_t headDim,
    int32_t elementBytes)
{
    ...
    for (int64_t index = first; index < count; index += stride)
    {
        ...
        int64_t const valueOffset =
            ((static_cast<int64_t>(batch) * sequenceLength * 3 + 2)
                    * channels
                + static_cast<int64_t>(head) * headDim)
            * elementBytes;
        int64_t const outputOffset =
        ...
        attentionPointers[index] = attentionBytes
            + static_cast<int64_t>(index) * sequenceLength * sequenceLength
                * elementBytes;
...
template <typename T>
__device__ __forceinline__ float scalarToFloat(T value);

template <>
__device__ __forceinline__ float scalarToFloat<float>(float value)
{
    return value;
}

template <>
__device__ __forceinline__ float scalarToFloat<half>(half value)
{
    return __half2float(value);
}

template <>
__device__ __forceinline__ float scalarToFloat<__nv_bfloat16>(
    __nv_bfloat16 value)
{
...
template <typename T>
__device__ __forceinline__ T floatToScalar(float value);

template <>
__device__ __forceinline__ float floatToScalar<float>(float value)
{
    return value;
}

template <>
__device__ __forceinline__ half floatToScalar<half>(float value)
{
    return __float2half(value);
}

template <>
__device__ __forceinline__ __nv_bfloat16 floatToScalar<__nv_bfloat16>(
    float value)
{
...
__global__ void parakeetRelativeSoftmax(T const* positionScores,
    int32_t const* validLengths, T const* contentScores, T* attentionWeights,
    int32_t batchSize, int32_t numHeads, int32_t sequenceLength,
    int32_t relativeLength,
    float scaleLog2)
{
...
#pragma unroll
    ...
    {
        int32_t const key = lane + slot * kWarpSize;
        float const value = slot < numSlots && key < sequenceLength
            ? exp2f(values[slot] - maximum)
            : 0.0F;
        values[slot] = value;
        localSum += value;
    }
...
#define LAUNCH_SOFTMAX_CASE(SLOTS, TYPE) \
    case SLOTS: \
        parakeetRelativeSoftmax<TYPE, SLOTS><<< \
            blocks, kThreadsPerBlock, 0, stream>>>( \
            static_cast<TYPE const*>(positionScores), validLengths, \
            static_cast<TYPE const*>(contentScores), \
            static_cast<TYPE*>(attentionWeights), batchSize, numHeads, \
            sequenceLength, relativeLength, scaleLog2); \
        break

template <typename T>
bool launchParakeetSoftmax(void const* positionScores,
    int32_t const* validLengths, void const* contentScores,
    void* attentionWeights, int32_t batchSize, int32_t numHeads,
    ...
    cudaStream_t stream)
{
    ...
    uint32_t const blocks =
        static_cast<uint32_t>((rows - 1) / kWarpsPerBlock + 1);
...
public:
        ...
        : mParameters(parameters), mTactic(tactic)
    {
        int const timingCacheLength = std::snprintf(mTimingCacheId.data(),
            mTimingCacheId.size(), "%s;scale=%a", kTimingCacheId,
            static_cast<double>(parameters.scale));
        bool const validTimingCacheId = timingCacheLength > 0
            && static_cast<size_t>(timingCacheLength) < mTimingCacheId.size();
        mInitialized = validTimingCacheId
    ...
    IPluginV3* clone() noexcept override
    {
        auto* plugin = new (std::nothrow)
            ParakeetFlashAttentionPlugin(mParameters, mTactic);
        ...
        int32_t nbOutputs) noexcept override
    {
        ...
        for (int32_t index = 0; index < 4; ++index)
        {
            if (inputs[index].desc.type != inputs[0].desc.type
                ...
                || !hasAddressableBytes(inputs[index].max,
                    dataTypeBytes(inputs[0].desc.type)))
            {
                return 1;
            }
        }
        if (!hasAddressableBytes(inputs[4].max, sizeof(int32_t))
            || !hasAddressableBytes(outputs[0].max,
                dataTypeBytes(outputs[0].desc.type))
        ...
        IExprBuilder& exprBuilder) noexcept override
    {
        ...
        if (inputs == nullptr || outputs == nullptr || nbInputs != kInputCount
            ...
            || inputs[2].nbDims != 2 || inputs[3].nbDims != 2
            || inputs[4].nbDims != 1 || inputs[0].d[0] == nullptr
            || inputs[0].d[1] == nullptr || inputs[1].d[2] == nullptr)
        {
            return 1;
        }
        outputs[0].nbDims = 3;
        ...
        int32_t nbOutputs) const noexcept override
    {
        ...
        WorkspaceLayout layout{};
        return computeWorkspaceLayout(
                   inputs[0].max, inputs[1].max, inputs[2].max,
                   inputs[0].desc.type, layout)
            ? layout.totalBytes
            : 0;
    }
        ...
        PluginTensorDesc const* outputs, int32_t nbOutputs) noexcept override
    {
        ...
        return haveValidShapes(inputShapes, outputs[0].dims, inputs[0].type)
            ? 0
            : 1;
    }
        ...
        cudaStream_t stream) noexcept override
    {
        ...
        cudaDataType_t const dataType = inputType == DataType::kHALF
            ? CUDA_R_16F
            : inputType == DataType::kBF16 ? CUDA_R_16BF : CUDA_R_32F;
        int32_t const pointerCount = batch * heads;

        WorkspaceLayout layout{};
        if (!computeWorkspaceLayout(
                inputDesc[0].dims, inputDesc[1].dims, inputDesc[2].dims,
                inputType, layout))
        {
            return 1;
        }
        ...
        auto pointerArray = [pointerArrays, pointerCount](int32_t index) {
            return pointerArrays + static_cast<size_t>(index)
                * static_cast<size_t>(pointerCount) * sizeof(void*);
        };
        auto** positionPointers =
            reinterpret_cast<void const**>(pointerArray(0));
        auto** queryPointers =
            reinterpret_cast<void const**>(pointerArray(1));
        ...
        if (cublasWorkspace != mWorkspace
            && cublasSetWorkspace(
                   mCublas, cublasWorkspace, kCublasWorkspaceBytes)
        ...
        int64_t const queryElements =
            static_cast<int64_t>(batch) * length * channels;
        uint32_t const queryBlocks = static_cast<uint32_t>(
            std::min<int64_t>(65535,
                (queryElements + threads - 1) / threads));
        if (inputType == DataType::kHALF)
        {
            prepareQueriesAndPositionPointers<<<queryBlocks, threads, 0, stream>>>(
                ...
                static_cast<half*>(contentQuery),
                static_cast<half*>(positionQuery),
                static_cast<half*>(positionScores), positionPointers,
                queryPointers, outputPointers, batch, heads, length,
                relativeLength, channels, headDim);
        }
        else if (inputType == DataType::kBF16)
        {
            prepareQueriesAndPositionPointers<<<queryBlocks, threads, 0, stream>>>(
                ...
                static_cast<__nv_bfloat16*>(positionScores), positionPointers,
                queryPointers, outputPointers, batch, heads, length,
                relativeLength, channels, headDim);
        }
        else
        {
            prepareQueriesAndPositionPointers<<<queryBlocks, threads, 0, stream>>>(
                ...
                static_cast<float*>(contentQuery),
                static_cast<float*>(positionQuery),
                static_cast<float*>(positionScores), positionPointers,
                queryPointers, outputPointers, batch, heads, length,
                relativeLength, channels, headDim);
        }
        if (cudaPeekAtLastError() != cudaSuccess)
        ...
        cublasStatus_t status = cublasGemmBatchedEx(mCublas,
            CUBLAS_OP_T, CUBLAS_OP_N, relativeLength, length, headDim, &alpha,
            positionPointers, dataType, channels, queryPointers, dataType,
            headDim, &beta, outputPointers, dataType, relativeLength,
        ...
        uint32_t const pointerBlocks = static_cast<uint32_t>(
            std::min<int64_t>(65535,
                (static_cast<int64_t>(pointerCount) + threads - 1) / threads));
        initializeContentPointers<<<pointerBlocks, threads, 0, stream>>>(
            inputs[0], contentQuery, attentionWeights, positionPointers,
            queryPointers, outputPointers, batch, heads, length, channels,
            headDim, elementBytes);
        if (cudaPeekAtLastError() != cudaSuccess)
        {
            return 1;
        }
        ...
        status = cublasGemmBatchedEx(mCublas, CUBLAS_OP_T, CUBLAS_OP_N,
            length, length, headDim, &alpha, positionPointers, dataType,
            3 * channels, queryPointers, dataType, headDim, &beta,
            outputPointers, dataType, length, pointerCount, computeType,
            CUBLAS_GEMM_DEFAULT);
        if (status != CUBLAS_STATUS_SUCCESS)
        {
            return 1;
        ...
        if (inputType == DataType::kHALF)
        {
            if (!launchParakeetSoftmax<half>(positionScores,
                static_cast<int32_t const*>(inputs[4]), attentionWeights,
                attentionWeights, batch, heads, length,
                relativeLength, mParameters.scale, stream))
            {
                return 1;
            }
        }
        ...
        initializeValuePointers<<<
            pointerBlocks, threads, 0, stream>>>(inputs[0],
            attentionWeights, outputs[0], positionPointers, queryPointers,
            outputPointers, batch, heads, length, channels, headDim,
            elementBytes);
        if (cudaPeekAtLastError() != cudaSuccess)
        {
            return 1;
        ...
        status = cublasGemmBatchedEx(mCublas, CUBLAS_OP_N, CUBLAS_OP_N,
            headDim, length, length, &alpha, positionPointers, dataType,
            3 * channels, queryPointers, dataType, length, &beta,
            outputPointers, dataType, channels, pointerCount,
...
private:
    ...
    {
        mSerializedFields[0] = {
            kScaleField, &mParameters.scale, PluginFieldType::kFLOAT32, 1};
...
public:
    ...
    {
        mAttributes[0] = {
            kScaleField, nullptr, PluginFieldType::kFLOAT32, 1};
        mFields = {
        ...
        TensorRTPhase phase) noexcept override
    {
        ...
        auto* plugin =
            new (std::nothrow) ParakeetFlashAttentionPlugin(parameters);
        if (plugin == nullptr || !plugin->mInitialized)
        {
            delete plugin;
            return nullptr;
...
extern "C" bool initFastGpuAsrParakeetFlashAttentionPlugin() noexcept
{
    ...
    auto ensureRegistered = [](IPluginRegistry* registry,
                                ParakeetFlashAttentionPluginCreator& creator) noexcept {
        if (registry == nullptr)
        {
            return false;
        }
